...
def save_game_state(board):
    with open("2048_game_state.pickle", "wb") as save_file:
        pickle.dump(board, save_file)

# Function to load the game state from a file
def load_game_state():
    try:
        with open("2048_game_state.pickle", "rb") as save_file:
            board = pickle.load(save_file)
        return board
    except FileNotFoundError:
        return None

# Function to delete the saved game state file
def delete_saved_game_state():
    if os.path.exists("2048_game_state.pickle"):
        os.remove("2048_game_state.pickle")
# %%
